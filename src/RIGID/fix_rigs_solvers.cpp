...
void FixRigs::shake3angle_solve(int i0, int i1, int i2, int ilist)
{
  ...
  Mat2 rPs(dot(r01, s01), dot(r01, s02),
           dot(r02, s01), dot(r02, s02));
  rPs = rr_inv * rPs;
  ...
  static const Mat2 J(0.0, -1.0, 1.0, 0.0);
...
void FixRigs::solve3x3(int ilist, Topology topo)
{
  ...
  SymMat3 sigma = Lsq;
  UsL3(sigma, mass_ldu);
  UTMat3 phi_new = mul_du(chol_upper(sigma), mass_ldu);
  ...
  Mat3 lamda = cayley_converge(phi_new, rnorm_new, chi, max_iter, tolerance, &niter);
  if (output_every) {
