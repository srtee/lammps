...
/* ----------------------------------------------------------------------
   ...
   12-6-4 extension based on the lj1264/cut/coul/long pair style
 ------------------------------------------------------------------------- */

#include "pair_lj1264_cut_tip4p_long.h"
...
PairLJ1264CutTIP4PLong::PairLJ1264CutTIP4PLong(LAMMPS *lmp) :
  PairLJ1264CutCoulLong(lmp)
{
  tip4pflag = 1;
...
PairLJ1264CutTIP4PLong::~PairLJ1264CutTIP4PLong()
{
...
void PairLJ1264CutTIP4PLong::compute(int eflag, int vflag)
{
  ...
  double r,r2inv,r4inv,r6inv,forcecoul,forcelj,cforce;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      // LJ + C4 interaction based on true rsq
      ...
      if (rsq < cut_ljsq[itype][jtype]) {
        ...
        forcelj = r6inv * (lj1[itype][jtype]*r6inv - lj2[itype][jtype])
                - 4.0*c4[itype][jtype]*r4inv*r2inv;
...
void PairLJ1264CutTIP4PLong::settings(int narg, char **arg)
{
...
void PairLJ1264CutTIP4PLong::coeff(int narg, char **arg)
{
  ...
  PairLJ1264CutCoulLong::coeff(narg, arg);
}
...
void PairLJ1264CutTIP4PLong::init_style()
{
  if (atom->tag_enable == 0)
    error->all(FLERR,"Pair style lj1264/cut/tip4p/long requires atom IDs");
  if (!force->newton_pair)
    error->all(FLERR,
               "Pair style lj1264/cut/tip4p/long requires newton pair on");
  if (!atom->q_flag)
    error->all(FLERR,
               "Pair style lj1264/cut/tip4p/long requires atom attribute q");
  if (force->bond == nullptr)
    error->all(FLERR,"Must use a bond style with TIP4P potential");
  if (force->angle == nullptr)
    error->all(FLERR,"Must use an angle style with TIP4P potential");

  PairLJ1264CutCoulLong::init_style();
...
void PairLJ1264CutTIP4PLong::write_restart_settings(FILE *fp)
{
...
void PairLJ1264CutTIP4PLong::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairLJ1264CutTIP4PLong::compute_newsite(double *xO, double *xH1,
...
void *PairLJ1264CutTIP4PLong::extract(const char *str, int &dim)
{
  dim = 0;
...
double PairLJ1264CutTIP4PLong::memory_usage()
{
